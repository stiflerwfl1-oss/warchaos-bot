...
# Valores reais no data.js: "marca", "insignia", "fita"
TYPE_MAP = {
...
async def enviar_resultados(ctx, mensagem_status, resultados, nome_busca):
    if not resultados:
...
# !conquista / !conquistas / !fita / !fitas -> busca por nome, sem filtro de tipo
@bot.command(aliases=['conquistas', 'fita', 'fitas'])
...
# !insignia -> busca por nome, filtra SOMENTE insignias
@bot.command()
async def insignia(ctx, *, nome_busca: str):
    mensagem_status = await ctx.send("Consultando o banco de dados do WarChaos...")
    try:
        dados = await carregar_dados()
        if dados is None:
...
# !marca -> busca por nome, filtra SOMENTE marcas
@bot.command()
async def marca(ctx, *, nome_busca: str):
    mensagem_status = await ctx.send("Consultando o banco de dados do WarChaos...")
    try:
        dados = await carregar_dados()
        if dados is None:
...
# !tudo / !todos -> busca por nome em TODOS os tipos
@bot.command(aliases=['todos'])
async def tudo(ctx, *, nome_busca: str):
    mensagem_status = await ctx.send("Consultando o banco de dados do WarChaos...")
    try:
        dados = await carregar_dados()
        if dados is None:
